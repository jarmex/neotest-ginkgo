...
function M.fail_fast(pos)
  local msg = "The selected folder must contain a go.mod file " .. "or be a subdirectory of a Go module."
...
function M.find_file_upwards(filename, start_path)
  ...
  while start_path ~= cwd do
    local files = scan.scan_dir(start_path, { search_pattern = filename, hidden = true, depth = 1 })
    if #files > 0 then
